...
const THIN_BORDER = {
  ...
  left: { style: 'thin' as const, color: { argb: 'FFD7DFE8' } },
  bottom: { style: 'thin' as const, color: { argb: 'FFD7DFE8' } },
  right: { style: 'thin' as const, color: { argb: 'FFD7DFE8' } }
}
...
export function mapHistoryEventToExportRow(item: HistoryEventItem): string[] {
  return [
    ...
    displayValue(handlingStatusDisplay(item.handlingStatus)),
...
function parseMergeStartRow(mergeRange: string) {
  const match = mergeRange.match(/^[A-Z]+(\d+):/i)
  return match ? Number(match[1]) : Number.POSITIVE_INFINITY
}

function prepareExcelSheet(sheet: import('exceljs').Worksheet, rowCount: number) {
  const columnCount = HISTORY_EVENT_EXPORT_HEADERS.length
  const clearUntilRow = Math.max(sheet.rowCount, DATA_START_ROW + rowCount + 2)

  const mergeRanges = [...(sheet.model.merges ?? [])]
  for (const mergeRange of mergeRanges) {
    if (parseMergeStartRow(mergeRange) >= DATA_START_ROW) {
      sheet.unMergeCells(mergeRange)
    }
  }

  HISTORY_EVENT_EXPORT_HEADERS.forEach((header, index) => {
    const cell = sheet.getCell(HEADER_ROW, index + 1)
    cell.value = header
    cell.font = { name: '宋体', size: 11, bold: true, color: { argb: 'FFFFFFFF' } }
    cell.fill = {
      type: 'pattern',
      pattern: 'solid',
  ...
  for (let rowIndex = DATA_START_ROW; rowIndex <= clearUntilRow; rowIndex += 1) {
...
async function exportHistoryEventsExcel(
  ...
  prepareExcelSheet(sheet, rows.length)

  rows.forEach((item, index) => {
    const values = mapHistoryEventToExportRow(item)
    values.forEach((value, colIndex) => {
      sheet.getCell(DATA_START_ROW + index, colIndex + 1).value = value
    })
  })
