...
OUTPUT_PATH = OUTPUT_DIR / "设备处置时间链实现说明.docx"
...
run = subtitle.add_run("飞行记录详情页 - 阶段语义、展示形态、状态规则与场景模板")
set_east_asia_font(run)
run.font.size = Pt(10.5)
...
meta_table = add_table(
    ...
    [
        ...
        ["文档用途", "用于产品确认、前端实现、脚本文案生成与演示数据维护"],
...
add_table(
    ...
    [
        ...
        ["阶段卡片", "标题 + 时间 + 状态标签 + 概要 + 详情 + 业务标签", "保持每个阶段结构一致，便于脚本生成。"],
        ["状态标签", "已完成 / 进行中 / 未触发 / 未开始", "标签用于阶段执行状态，不承担业务结论本身。"],
        ["业务标签", "来源、身份、风险、处置、结果", "由字段脚本派生，例如雷达、黑名单、高风险、未触发预案。"],
        ["未开始节点", "默认隐藏", "详情页面向既有事件记录，隐藏未发生内容可减少误解。"],
        ["未触发节点", "可展示为轻量节点", "例如飞鸟扰动、白名单低风险、未命中预案等，需要解释为什么没有反制。"],
    ],
    [1700, 2300, 5360],
)
...
add_code_block(
    document,
    """type TimelineStageStatus = 'completed' | 'processing' | 'skipped' | 'pending'
...
add_table(
    ...
    ["状态", "页面文案", "触发条件", "展示规则"],
    [
        ["completed", "已完成", "阶段已发生并产生结构化结论。", "正常展示完整节点。"],
        ["processing", "进行中", "前置阶段已完成，本阶段正在执行或等待回传。", "展示当前节点，后续 pending 节点隐藏。"],
...
add_body(
    document,
    "处置执行阶段是否展示取决于业务解释价值：已触发反制时完整展示；未命中预案但需要说明原因时展示为“未触发”；仅因流程未走到此处时隐藏。"
)
...
add_table(
    ...
    [
        ...
        ["业务标签", "字段派生标签", "{deviceType}、{identityLabel}、{zoneName}、{planHitStatus}"],
    ],
    [1700, 3200, 4460],
)
add_code_block(
...
add_table(
    ...
    ["标签类别", "示例", "派生条件"],
    [
        ["来源标签", "雷达、无线电、多源融合", "来自 discovery.sources 或 dataSource 字段。"],
        ["身份标签", "非合作式、黑名单、白名单、飞鸟扰动", "来自识别阶段 targetType、cooperationType、listStatus。"],
        ["风险标签", "高风险、核心区、命中规则", "来自 assessment.threatLevel、zoneName、hitRules。"],
        ["处置标签", "已联动反制、未触发预案、指令下发", "来自 planHit、countermeasureDevice、commandStatus。"],
        ["结果标签", "已处置、已结束、目标自离、误报排除", "来自 handlingStatus、handlingResult、endReason。"],
    ],
    [1700, 2600, 5060],
)
...
scenario_rows = [
    [
        "黑飞无人机入侵",
        "非合作式无人机进入防护区域。",
        "设备发现=已完成；威胁识别=已完成；威胁评估=已完成；处置执行=已完成/进行中；处置结果=已完成/进行中。",
        "展示完整五阶段。若反制执行中，处置结果暂不展示或展示为进行中。",
    ],
    [
        "黑名单无人机入侵",
        "合作式或品牌无人机命中黑名单后进入区域。",
        "设备发现=已完成；威胁识别=已完成；威胁评估=已完成；处置执行=已完成；处置结果=已完成。",
        "展示完整五阶段，识别阶段突出黑名单命中，评估阶段突出高风险规则。",
    ],
    [
        "白名单无人机",
        "合作式白名单目标进入授权或低风险区域。",
        "设备发现=已完成；威胁识别=已完成；威胁评估=已完成；处置执行=未触发；处置结果=已完成。",
        "处置执行展示为轻量未触发节点，说明白名单或低风险未命中反制预案。",
    ],
    [
        "飞鸟扰动",
        "探测信号经识别判定为飞鸟或非无人机目标。",
        "设备发现=已完成；威胁识别=已完成；威胁评估=已完成；处置执行=未触发；处置结果=已完成。",
        "处置结果使用“已结束”，结束原因可为飞鸟扰动排除或误报排除。",
    ],
]
add_table(
    document,
...
add_table(
    ...
    [
        ...
        ["飞鸟扰动", "威胁识别", "已完成", "[2026-06-25 17:31:11] 完成目标识别，判定为飞鸟扰动，身份属性为非无人机目标。"],
        ["飞鸟扰动", "处置结果", "已完成", "[2026-06-25 17:31:42] 事件已闭环，最终结果为飞鸟扰动排除。"],
    ],
    [1600, 1500, 1200, 5060],
    font_size=8.3,
)
...
add_heading(document, "十二、待确认事项", 1)
add_bullet(document, "白名单目标在未授权区域或异常飞行时，是否允许由威胁评估升级并触发处置预案。")
add_bullet(document, "飞鸟扰动是否统一归入“已结束”，还是细分为“误报排除”。")
add_bullet(document, "进行中事件是否展示处置结果的 processing 节点，或仅展示到处置执行阶段。")
add_bullet(document, "横向步骤条是否作为详情页顶部概览保留，还是仅使用纵向时间线。")
